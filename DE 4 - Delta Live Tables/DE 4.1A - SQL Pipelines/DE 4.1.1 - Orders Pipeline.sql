...
-- MAGIC %md
-- MAGIC
-- MAGIC Which of the following correctly describes how Auto Loader ingests data? Select one response.
-- MAGIC
-- MAGIC - Auto Loader automatically detects new data files during manual or scheduled updates.
-- MAGIC
-- MAGIC - Auto Loader only detects new data files during scheduled update intervals.
-- MAGIC
-- MAGIC - Auto Loader writes new data files in incremental batches.
-- MAGIC
-- MAGIC - Auto Loader incrementally ingests new data files in batches.
-- MAGIC
-- MAGIC - Auto Loader automatically writes new data files continuously as they land.

-- COMMAND ----------

-- MAGIC %md
-- MAGIC
-- MAGIC A data engineer wants to query metrics on the latest update made to their pipeline. The pipeline has multiple data sources. Despite the input data sources having low data retention, the data engineer needs to retain the results of the query indefinitely.
-- MAGIC
-- MAGIC Which of the following statements identifies the type of table that needs to be used and why? Select one response.
-- MAGIC
-- MAGIC - Streaming live table; streaming live tables can preserve data indefinitely.
-- MAGIC
-- MAGIC - Streaming live table; streaming live tables record live metrics on the query.
-- MAGIC
-- MAGIC - Streaming live table; streaming live tables are always "correct", meaning their contents will match their definition after any update.
-- MAGIC
-- MAGIC - Live table; live tables retain the results of a query for up to 30 days.
-- MAGIC
-- MAGIC - Live table; live tables only support reading from "append-only" streaming sources.

-- COMMAND ----------

-- MAGIC %md
-- MAGIC
-- MAGIC A data engineer has created the following query to create a streaming live table from **transactions**.
-- MAGIC
-- MAGIC Code block:
-- MAGIC
-- MAGIC CREATE OR REFRESH STREAMING LIVE TABLE transactions
-- MAGIC
-- MAGIC AS SELECT timestamp(transaction_timestamp) AS transaction_timestamp, * EXCEPT (transaction_timestamp, source)
-- MAGIC
-- MAGIC __
-- MAGIC
-- MAGIC Which of the following lines of code correctly fills in the blank? Select two responses.
-- MAGIC
-- MAGIC - FROM DELTA STREAM(LIVE.transactions)
-- MAGIC - FROM STREAMING LIVE (transactions)
-- MAGIC - FROM STREAM(LIVE.transactions)
-- MAGIC - FROM LIVE.transactions
-- MAGIC - FROM STREAMING LIVE.transactions
...
-- MAGIC %md
-- MAGIC
-- MAGIC A data engineer is running a Delta Live Tables (DLT) notebook. They notice that several commands display the following message:
-- MAGIC
...
-- MAGIC
-- MAGIC Which of the following statements explains this message? Select one response.
-- MAGIC
-- MAGIC - DLT is not intended for interactive execution in a notebook.
-- MAGIC
-- MAGIC - DLT does not support the execution of Python and SQL notebooks within a single pipeline.
-- MAGIC
-- MAGIC - DLT notebooks must be run at scheduled intervals using the job scheduler.
-- MAGIC
-- MAGIC - DLT queries must be connected to a pipeline using the pipeline scheduler.
-- MAGIC
-- MAGIC - DLT does not support the execution of Python commands.

-- COMMAND ----------

-- MAGIC %md
-- MAGIC
-- MAGIC A data engineer is creating a live streaming table to be used by other members of their team. They want to indicate that the table contains silver quality data.
-- MAGIC
-- MAGIC Which of the following describes how the data engineer can clarify this to other members of their team? Select two responses.
-- MAGIC
-- MAGIC - TBLPROPERTIES ("quality" = "silver")
-- MAGIC - EXPECT QUALITY = SILVER
-- MAGIC - WHEN QUALITY = SILVER THEN PASS
-- MAGIC - None of these answer choices are correct.
-- MAGIC - COMMENT "This is a silver table"
