...
-- MAGIC %md
-- MAGIC A data engineer has configured and deployed a DLT pipeline that contains an error. The error is thrown at the third stage of the pipeline, but since DLT resolves the order of tables in the pipeline at different steps, they are not sure if the first stage succeeded.
-- MAGIC
-- MAGIC Which of the following is considered a good practice to determine this? Select one response.
-- MAGIC
-- MAGIC - The data engineer can fix the tables from the Directed Acyclic Graph (DAG), starting at the dataset containing the error.
-- MAGIC
-- MAGIC - The data engineer can fix one table at a time, starting at their earliest dataset.
-- MAGIC
-- MAGIC - The data engineer can fix the tables using iterative logic, starting at the dataset containing the error.
-- MAGIC
-- MAGIC - The data engineer can fix the tables using iterative logic, starting at their earliest dataset.
-- MAGIC
-- MAGIC - The data engineer can fix the tables from the Directed Acyclic Graph (DAG), starting at their earliest dataset.

-- COMMAND ----------

-- MAGIC %md
-- MAGIC
-- MAGIC A data engineer is using the code below to create a new table **transactions_silver** from the table **transaction_bronze**. However, when running the code, an error is thrown.
-- MAGIC
-- MAGIC ```sql
-- MAGIC CREATE OR REFRESH STREAMING LIVE TABLE transactions_silver
-- MAGIC
-- MAGIC (CONSTRAINT valid_date EXPECT (order_timestamp > "2022-01-01") ON VIOLATION DROP ROW)
-- MAGIC
-- MAGIC FROM LIVE.transactions_bronze
-- MAGIC ```
-- MAGIC Which of the following statements correctly identifies the error and the stage at which the error was thrown? Select one response.
-- MAGIC
-- MAGIC - A **SELECT** statement needs to be added to create the columns for the **transactions_silver** table. The error will be detected during the Initializing stage.
-- MAGIC
-- MAGIC - **LIVE.orders_bronze** needs to be changed to **STREAM(LIVE.orders_bronze)**. The error will be detected during the Setting Up Tables stage.
-- MAGIC
-- MAGIC - A **SELECT** statement needs to be added to create the columns for the **transactions_silver** table. The error will be detected during the Setting Up Tables stage.
-- MAGIC
-- MAGIC - **LIVE.orders_bronze** needs to be changed to **STREAM(LIVE.orders_bronze)**. The error will be detected during the Initializing stage.
-- MAGIC
-- MAGIC - The **EXPECT** statement needs to be changed to **EXPECT (order_timestamp is NOT NULL)**. The error will be detected during the Setting Up Tables stage.
