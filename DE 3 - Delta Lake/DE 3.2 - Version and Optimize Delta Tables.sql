...
-- MAGIC
-- MAGIC **1) Which of the following statements about vacuuming with Delta Lake is true? Select one response.**
-- MAGIC   1. Delta table metadata files will not be vacuumed unless the auto retention check is turned off.
-- MAGIC   2. On Delta tables, Databricks automatically triggers VACUUM operations as data is written.
-- MAGIC   3. Running VACUUM on a Delta table eliminates the ability to time travel back to a version older than the specified data retention period
-- MAGIC   4. Delta table data files are vacuumed according to their modification timestamps on the storage system.
-- MAGIC   5. VACUUM will not vacuum any directories that begin with an underscore except for _delta_log.
-- MAGIC
-- MAGIC **2) A data engineer has a collection of tables. They need to manually remove old data files from the tables and remove access to previous versions of the tables.**
-- MAGIC
-- MAGIC Which of the following approaches allows the data engineer to do this? Select one response.
-- MAGIC
-- MAGIC  
-- MAGIC 1. They need to disable the retention duration check and enable vacuum logging. Then they need to vacuum the table.
-- MAGIC They need to disable the retention duration check and enable the last commit version in session. Then they need to vacuum the table.
-- MAGIC 2. They need to enable the retention duration check and disable vacuum logging. Then they need to Z-order the table.
-- MAGIC 3. They need to disable the last commit version in session and enable vacuum duration check. Then they need to Z-order the table.
-- MAGIC 4. They need to enable the retention duration check and vacuum logging. Then they need to optimize the table.
